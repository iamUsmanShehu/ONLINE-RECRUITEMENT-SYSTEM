...
<?php
		...
		while($row=mysqli_fetch_array($query)) {
			...
			echo "
					 <tr>
				      ...
				      <td><a href=\"massage.php?id=$id&positionnames=$positionname\"><i class='fa fa-edit'></i></a></td>
				    </tr>				
			";
		...
		?>	
				...
				<table class="table table-hover" style="border:1px solid #040434; font-size: 14px;">
  <thead>
  	<tr>
      <th colspan="8" scope="col" class="vacancyhead"><center>APPLICANTS TABLE</center></th>
    </tr>
    <tr>
      <th scope="col"></th>
      <th scope="col"></th>
...
<?php
		...
		$query=mysqli_query($connect, "SELECT * FROM `applicants_tbl` \n" . " ORDER BY `id` DESC");
		...
		while($row=mysqli_fetch_array($query)) {
			...
			$address = $row["address"];
			$image = $row["image"];
			$image="<img src='../../pages/uploaded_imgs/". $image ."' width='50' height='50'>";
		...
		<div class="col-md-4">
				<form method="POST" action="">
					<div class="row"><?php if(isset($result)) echo "$result"; ?>
						<div class="vacancyhead"><h2><i class="fa fa-plus"></i> new vacancy</h2></div>
						
						<div class="col">
							<input type="text" name="positionname" class="form-control margin border" placeholder="Position">
						</div>
						<div class="col">
							<select type="text" name="status" class="form-control margin border">
								<option>select status...</option>
								<option class="active">Active</option>
								<option class="closed">Closed</option>
							</select>
						</div>
						<input type="text" name="availablity" class="form-control margin border" placeholder="Availability (eg. 30, 20, 2...)">
						<textarea type="text" name="description" class="form-control margin border"placeholder="Vacancy Description..."></textarea>
